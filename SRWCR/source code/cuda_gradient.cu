/*
   This source code is just for test, so that it is not corresponding to the standard style.
   If you have any problem, please do not hesitate to contract me: gonglun@hotmail.com
   ...
   The input:
		...
		gradientu-----the gradinet of SRWCR
		int_constant---{SIZEX,SIZEY,SIZEZ,nx,ny,nz}; the sizes of image and the numbers of control nodes 
        float_constant--{spacex,spacey,spacez};   the spacings of grid
        instance--the constants usded by lbfgs
...
{
   float *dev_in1,*dev_in2,*dev_Transform_Image,*dev_float_constant;
   ...
   uint *dev_pxyz;
   dev_pxyz=instance[0].dev_uint[1];  //the index of each voxel in 3-D coordinates
   ...
   float *dev_transformxyz;
   HANDLE_ERROR(cudaMalloc((void **)&dev_transformxyz,3*bit_image)); //store the coordinates of the transformed image 
  ...
  double *dev_var_block;
  float *dev_m_f;
  HANDLE_ERROR(cudaMalloc((void **)&dev_var_block,bit_grid_d));//these results will be used to compute the gradient
  HANDLE_ERROR(cudaMalloc((void **)&dev_m_f,sizeof(float)*grid_SIZE*bin));//these results will be used to compute the gradient
  HANDLE_ERROR(cudaMemcpy(dev_var_block,var_block,bit_grid_d,cudaMemcpyHostToDevice));   
  HANDLE_ERROR(cudaMemcpy(dev_m_f,m_f,sizeof(float)*grid_SIZE*bin,cudaMemcpyHostToDevice)); 
  free(var_block);
  free(m_f);
  float *dev_gradient_TI,*dev_gradient_image;
  ...
  if(grid_SIZE<MAX_gridsize) // judge if the number of control nodes exceeds the max block size 
  {
    blocks=grid_SIZE;
    threads=512;
...
__global__ void  cudaBspline_transform(float *in1,float *Transform_Image,int *int_constant,float *double_constant,float *BsplineX,float *BsplineY,float *BsplineZ,double *X,float *transformxyz,uint *pxyz,int mode)
{
   ...
   while(K<image_SIZE)
   {
	   ...
	   for(indx=0;indx<4;indx++)
        ...
        Transform_Image[K]=interpolate_3d_float_gray_GPU(sx, sy, sz, Isize, in1,cubic,black);  // resample the moving image
		transformxyz[K]=sx;
